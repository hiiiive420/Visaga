...
function playIntroMusic() {
  if (!introMusic || introMusicAttempted) return;
...
function burst() {
  ...
  shockwaves = [
    {
      r: 10,
      max: Math.max(innerWidth, innerHeight) * 0.9,
      a: 0.9,
      c: '#f3e5c8'
    },
    {
      r: 10,
      max: Math.max(innerWidth, innerHeight) * 0.7,
      a: 0.6,
      c: '#d4af37'
    }
  ];
  ...
  for (let i = 0; i < 90; i++) {
    ...
    particles.push({
      ...
      vy: Math.sin(a) * s - 1,
      w: 1 + Math.random() * 2,
      c: Math.random() > 0.4 ? '#fffbd4' : '#fff',
      rot: 0,
      vr: 0,
      o: 0.9,
      d: 0.009 + Math.random() * 0.008,
      wob: 0
    });
  }
...
function finish(skipAnimation = false) {
  ...
  if (skipAnimation) {
    ...
    intro.classList.add('reveal-out');
  ...
  }, skipAnimation ? 50 : 650);
}

document.body.style.overflow = 'hidden';

if (
  sessionStorage.getItem('syas-intro-seen') ||
